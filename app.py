...
def analyze_ticker_data(ticker_df, min_volume, vsa_lookback, spring_lookback, max_penetration):
    ...
    if env_pct <= -10 and current_rsi < 35: trend_context = f"🟢 Vùng Đảo Chiều Đáy (Env {env_pct:.1f}%)"
    elif env_pct >= 30 and current_rsi > 70: trend_context = f"🔴 Cảnh báo Đu đỉnh (Env +{env_pct:.1f}%)"
    elif is_long_term_trend and current_close > current_ma252: trend_context = "🚀 Tín hiệu vào Trend Dài Hạn"
    elif streak > 0 and streak < 21: trend_context = f"⏳ Chờ xác nhận Trend (Ngày {streak}/21)"
    ...
    for i in range(len(scan_window)):
        ...
        if signal:
            ...
            if signal in ["🟢 No Supply", "🔥 Spring"]: struct_eval = "🌟 Mua Vàng (Đã rũ SC)" if has_stopping_volume_past else "⏳ Sớm (Chưa có SC)"
            elif signal == "🔴 Stopping Vol": struct_eval = "👀 Watchlist (Đợi Test đáy)"
            elif signal == "🚀 SOS": struct_eval = "📈 Đánh Đẩy giá (Markup)"

    return {
        "Giá Hiện Tại": current_close, "Thanh Khoản (20đ)": f"{int(df['Vol_MA20'].iloc[-1]):,}",
        "Score_Current": score_current, "Score_Past": score_past,
        "VSA_Signal": latest_signal, "Đánh giá Cấu trúc": struct_eval,
        "Vị thế Trend (MA252)": trend_context, "Ngày Tín Hiệu": signal_date, "POE": poe, "SL": sl
    }

# --- KHỞI TẠO BỘ NHỚ ---
if 'app_data' not in st.session_state: st.session_state.app_data = None
...
if st.session_state.app_data is not None:
    if st.session_state.app_data.empty:
        ...
        st.info(f"⏱️ **Dữ liệu chốt lúc:** {st.session_state.last_update_time} (Thời gian quét: {st.session_state.scan_time}s).")
        df_results = st.session_state.app_data.copy()
        col_config_general = {"SCTR": st.column_config.NumberColumn("SCTR", format="%.1f"), "Giá Hiện Tại": st.column_config.NumberColumn("Giá Hiện Tại", format="%.0f")}
        ...
        if not df_signals.empty:
            if selected_sector_2 != "Tất cả": df_signals = df_signals[df_signals['Ngành'] == selected_sector_2]
        ...
        if selected_sector_3 != "Tất cả": df_ranking = df_ranking[df_ranking['Ngành'] == selected_sector_3]
        ...
        def highlight_trend(val):
            ...
            return ''
        st.dataframe(df_ranking[cols_rank].style.map(highlight_trend, subset=['Vị thế Trend (MA252)']), use_container_width=True, column_config=col_config_general)
        
        excel_rank = to_excel_safe(df_ranking[cols_rank])
        if excel_rank:
            st.download_button("📥 Tải Bảng Xếp Hạng (.xlsx)", data=excel_rank, file_name=f"XepHang_{datetime.datetime.now().strftime('%Y%m%d')}.xlsx", mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
